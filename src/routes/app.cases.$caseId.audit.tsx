...
function AuditPage() {
  const allApproved = AUDIT_DIMENSIONS.every(d => d.status === "APROVADA" || d.status === "NÃO SE APLICA");
    ...
    <div className="p-6 space-y-6 pb-32">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-white/40">Verificação de conformidade e integridade pericial.</p>
        </div>
        {allApproved && (
          <div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-full animate-pulse-glow">
            ...
            <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest">Auditoria 10/10</span>
          </div>
        )}
      </header>
      ...
      <div className="grid gap-3">
        {AUDIT_DIMENSIONS.map((dim) => (
          <div key={dim.id} className="bg-white/5 border border-white/5 rounded-2xl p-4 flex items-center justify-between group hover:bg-white/[0.07] transition-all">
...
function StatusBadge({ status }: { status: string }) {
  ...
  const Icon = status === "APROVADA" ? CheckCircle2 : status === "REPROVADA" ? XCircle : MinusCircle;

  return (
    <div className={cn("px-2 py-1 border rounded-md text-[8px] font-bold uppercase tracking-widest flex items-center gap-1.5", styles[status])}>
