...
function DocumentTypePage() {
  ...
  const selectedProfs = selectedProfIds
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-1">
            {[1, 2, 3, 4, 5].map((s) => (
              <div
                key={s}
                className={`w-8 h-1 rounded-full ${s === 3 ? "bg-veritas-electric" : s < 3 ? "bg-veritas-electric/40" : "bg-white/10"}`}
              />
            ))}
          </div>
          <div className="w-12" />
            ...
            >
              <div
                className={cn(
                  ...
                  selectedId === opt.id
                    ? "bg-veritas-violet border-transparent"
                    : "bg-white/5 border-white/10",
                )}
              >
                <FileText
                  className={cn("w-5 h-5", selectedId === opt.id ? "text-white" : "text-white/20")}
                />
              </div>
              <div className="flex-1">
                <span className="block text-sm font-medium">{opt.label}</span>
                <span className="block text-[10px] text-white/40 mt-0.5 uppercase tracking-wider font-bold">
                  {opt.discipline === "psychology"
                    ? "Psicologia"
                    : opt.discipline === "social-work"
                      ? "Serviço Social"
                      : "Multiprofissional"}
                </span>
              </div>
