...
import { ChevronLeft, Save, MessageSquare, Eye, Edit3, Check, X, RefreshCw, Search, Plus, Trash2, FileText, AlertCircle } from "lucide-react";
...
function DraftEditPage() {
  ...
  const [confirmDelete, setConfirmDelete] = useState<{ type: 'section' | 'paragraph', id: string } | null>(null);
    ...
    return (
      <div className="flex flex-col h-screen bg-veritas-ink items-center justify-center p-4">
        <div className="bg-white/5 border border-white/10 p-8 rounded-2xl max-w-md w-full text-center space-y-4">
          ...
          <p className="text-white/60 text-sm">Não foi possível carregar as informações do caso solicitado.</p>
          <Link to="/app/cases" className="inline-block px-6 py-2 bg-veritas-electric text-white rounded-lg font-bold">
            Voltar para Casos
          </Link>
        </div>
      </div>
  ...
  const handleUpdateParagraph = (paragraphId: string) => {
    ...
    updateParagraph(caseId, versionId, paragraphId, { text: paragraphTextDraft });
    ...
    <div className="flex flex-col min-h-screen bg-veritas-ink text-white">
      <header className="sticky top-0 z-50 bg-veritas-graphite border-b border-white/5 p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-[10px] font-bold text-white/40 uppercase tracking-widest leading-none mb-1">Editor de Relatório</h2>
            <p className="text-xs font-bold text-veritas-electric leading-none">Versão {workflow.currentVersion.label}</p>
          </div>
        </div>
        ...
        {!document ? (
          <div className="py-20 text-center space-y-6 max-w-sm mx-auto">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-white/40">Inicie um rascunho vazio para começar a estruturar o documento.</p>
            </div>
            <button 
            ...
            <div className="space-y-8">
              {document.sections.length === 0 ? (
                <div className="py-12 border-2 border-dashed border-white/5 rounded-2xl text-center space-y-4">
                  <p className="text-white/40 text-sm italic">Rascunho iniciado. Nenhuma seção criada.</p>
                </div>
              ) : (
                document.sections.map((section) => (
                  <section key={section.id} className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden shadow-sm">
                    {/* Section Header */}
                    <div className="bg-white/5 px-6 py-4 flex items-center justify-between border-b border-white/5">
                      {editingSectionId === section.id ? (
                        <div className="flex-1 flex gap-2">
                          ...
                          />
                          <button onClick={() => handleUpdateSection(section.id)} className="p-1.5 bg-veritas-electric text-white rounded"><Check className="w-4 h-4" /></button>
                          <button onClick={() => setEditingSectionId(null)} className="p-1.5 bg-white/10 text-white/60 rounded"><X className="w-4 h-4" /></button>
                        </div>
                      ) : (
                        <>
                          <div className="flex items-center gap-3">
                            <span className="text-[10px] font-mono text-white/20 uppercase tracking-widest">{section.id}</span>
                    ...
                    <div className="p-6 space-y-6">
                      {section.paragraphs.map((para) => (
                        <div key={para.id} className="group relative bg-white/[0.02] p-4 rounded-xl border border-transparent hover:border-white/5 transition-all">
                          <div className="flex items-start justify-between mb-2">
                            <div className="flex items-center gap-2">
                              <span className="text-[10px] font-mono text-white/20 uppercase tracking-widest">{para.id}</span>
                              {para.traceability && (
                            ...
                            {editingParagraphId !== para.id && (
                              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <button 
                                  onClick={() => setConfirmDelete({ type: 'paragraph', id: para.id })}
                          ...
                          {editingParagraphId === para.id ? (
                            ...
                            <p className="text-sm text-white/80 leading-relaxed whitespace-pre-wrap">{para.text}</p>
                          )}
                        </div>
                      ))}
                      ...
                      <div className="pt-4 border-t border-white/5">
                        <textarea 
                          ...
                          onChange={(e) => setNewParagraphTexts(prev => ({ ...prev, [section.id]: e.target.value }))}
                        />
              ...
              <div className="bg-veritas-electric/5 border border-veritas-electric/20 rounded-2xl p-6 space-y-4">
                <div className="flex items-center gap-2 text-veritas-electric mb-2">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-widest">Nova Seção</span>
                </div>
