...
function InspectionPage() {
  ...
  const hasContent = document !== undefined && document.sections.length > 0;
    ...
    <div className="p-6 space-y-8 pb-32 overflow-y-auto h-full bg-veritas-ink">
      <header className="flex items-center gap-3">
        <Link
          ...
          aria-label="Voltar para documento final"
        >
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-12 text-center space-y-6 max-w-2xl mx-auto w-full min-w-0">
          ...
          <div className="space-y-2">
            <h3 className="text-white font-bold">
              Nenhum documento disponível para inspeção nesta versão.
            </h3>
            <p className="text-sm text-white/40">
              Crie e revise o rascunho antes de realizar a inspeção visual.
            </p>
          </div>
          <Link
            ...
            <div className="bg-veritas-graphite border border-white/5 p-6 rounded-2xl space-y-6">
              ...
              <div className="grid gap-2">
                {INSPECTION_ITEMS.map((item) => (
                  <button
                    key={item}
                    onClick={() => toggle(item)}
                    aria-pressed={checked[item] === true}
                    className={cn(
              ...
              >
                {allChecked ? (
                  ...
                  <>
                    <Inspect className="w-8 h-8 text-white/10" aria-hidden="true" />
