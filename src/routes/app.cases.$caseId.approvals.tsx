    ...
    <div className="p-6 space-y-6 pb-32">
      <header className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold text-white tracking-tight">Portão de Aprovações</h2>
          <div className="bg-veritas-electric/10 border border-veritas-electric/20 px-4 py-1.5 rounded-full">
            <span className="text-sm font-black text-veritas-electric">{approvedCount}/4</span>
          </div>
        </div>
        <p className="text-xs text-white/40 leading-relaxed">
          Para liberar a emissão do documento final, todos os pilares de segurança e conformidade devem estar validados.
        </p>
      </header>
        ...
        {approvals.map((approval) => {
          ...
          return (
            <div key={approval.id} className={cn(
              ...
              approval.status ? "border-emerald-500/30 bg-emerald-500/[0.03]" : "border-white/5"
            )}>
              <div className="flex items-center gap-4">
                <div className={cn(
                  "w-10 h-10 rounded-xl flex items-center justify-center",
                  approval.status ? "bg-emerald-500/10 text-emerald-400" : "bg-white/5 text-white/20"
                )}>
                ...
                <div>
                  ...
                  <p className="text-[9px] uppercase tracking-widest font-bold opacity-40">Verificação Independente</p>
                </div>
              </div>
