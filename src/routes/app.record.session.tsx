...
function RecordSessionPage() {
  ...
  const markerButtons = [
    { label: "Importante", color: "bg-veritas-violet/20 text-veritas-violet border-veritas-violet/30", icon: <Flag className="w-4 h-4" /> },
    { label: "Conferir", color: "bg-blue-500/20 text-blue-400 border-blue-500/30", icon: <Info className="w-4 h-4" /> },
    { label: "Contradição", color: "bg-red-500/20 text-red-400 border-red-500/30", icon: <AlertCircle className="w-4 h-4" /> },
    { label: "Observação", color: "bg-amber-500/20 text-amber-400 border-amber-500/30", icon: <MessageSquare className="w-4 h-4" /> },
    { label: "Sensível", color: "bg-pink-500/20 text-pink-400 border-pink-500/30", icon: <Eye className="w-4 h-4" /> },
  ];
    ...
    <div className="min-h-[100dvh] bg-veritas-graphite-dark pb-[calc(6rem+env(safe-area-inset-bottom))] text-white flex flex-col relative overflow-hidden">
          ...
          {waveforms.map((h, i) => (
            <div 
              ...
              className={cn(
                ...
                isPaused ? "bg-veritas-silver/10" : "bg-veritas-electric shadow-[0_0_15px_rgba(30,174,255,0.3)]"
              )}
              style={{ height: `${h}%` }}
            />
          ))}
      ...
      <div className="px-6 pt-8 pb-12 text-center space-y-10 relative z-10">
        <div className="space-y-1">
          ...
          <p className="text-[10px] text-veritas-silver/30 font-bold uppercase tracking-widest">Duração da sessão</p>
        </div>
      ...
      <section className="px-6 space-y-4 relative z-10">
        <div className="flex items-center justify-between">
          <h3 className="text-[10px] font-bold text-veritas-silver/40 uppercase tracking-widest">Marcadores Rápidos</h3>
          <span className="text-[10px] font-mono text-veritas-electric/60">{markers.length} registrados</span>
        </div>
