  ...
  const preferences = [
    {
      label: "Tema escuro",
      icon: Moon,
      description: "Ativar interface Graphite Dark.",
      type: "switch",
      value: true,
    },
    {
      ...
      icon: Bell,
      description: "Alertas de prazos e transcrições.",
      type: "switch",
      value: true,
    },
    {
      ...
      icon: Download,
      description: "Gerenciar atalho no dispositivo.",
      type: "action",
    },
    ...
    <div className="min-h-[100dvh] veritas-hero-gradient pb-[calc(6rem+env(safe-area-inset-bottom))] text-white relative">
      ...
      <main className="p-6 space-y-4">
        {preferences.map((pref) => (
          <div
            key={pref.label}
            className="p-5 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-between group hover:bg-white/[0.08] transition-all"
          >
            ...
            {pref.type === "switch" ? (
              ...
              <Button
                variant="ghost"
                size="sm"
                className="text-veritas-electric text-[10px] font-bold uppercase tracking-widest"
              >
                Gerenciar
              </Button>
            )}
