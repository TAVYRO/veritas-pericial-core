...
import { Flag, AlertTriangle, Lock, Unlock, ChevronRight, HelpCircle } from "lucide-react";
...
const GAPS = [
  {
    ...
    question:
      "Qual o horário exato em que a criança foi entregue à genitora no domingo, dia 09/08/2026?",
    reason:
      "A divergência de 2 horas impede a análise do impacto temporal da visita na agitação relatada.",
    status: "blocked",
  },
  {
    type: "BLOQUEIA ELABORAÇÃO",
    ...
    reason:
      "Contradição entre relato da genitora e evidências do Estudo Social compromete a análise da rede de apoio.",
    status: "blocked",
  },
  {
    ...
    reason:
      "Informação relevante para o plano de convivência, mas não impede a conclusão diagnóstica da rotina atual.",
    status: "pending",
  },
];
    ...
    <div className="p-6 space-y-6 pb-24">
      ...
      <div className="space-y-4">
        {GAPS.map((gap, idx) => (
          ...
          >
            ...
            <p className="text-sm text-white/90 font-medium leading-relaxed">{gap.question}</p>

            <div className="bg-black/20 rounded-xl p-3">
              <p className="text-[10px] text-white/40 uppercase tracking-widest mb-1">
                Impacto na Análise
              </p>
              <p className="text-xs text-white/60 leading-relaxed italic">{gap.reason}</p>
            </div>
      ...
      <div className="p-4 bg-rose-500/5 border border-rose-500/10 rounded-xl flex gap-3">
        ...
        <p className="text-[10px] text-rose-500/80 leading-relaxed">
          Existem 2 lacunas que <strong>bloqueiam</strong> a progressão para a etapa de Auditoria e
          Fechamento. A Veritas recomenda a resolução destas questões antes da redação final.
        </p>
      </div>
    </div>
  );
